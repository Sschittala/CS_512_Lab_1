...
\stuinfo{\textcolor{b}{SEE BELOW}}{\textcolor{b}{SEE BELOW}}
\graphicspath{{./}{./Figures/}}

\begin{document}
\maketitle \\
\textcolor{b}{\textbf{Students:} \text{Tyler Strach, Miles Woodcock-Girard, Sai Chittala}} \\
\textcolor{b}{\textbf{Emails:} \text{tstra6@uic.edu, mwoodc2@uic.edu, schit7@uic.edu}}
...
\begin{itemize}
	...
	\textcolor{b}{(1a) derivation is shown on the following 2 pages.}
	\includepdf[pages=-]{lab1_1a.pdf}
	...
	\textcolor{b}{(1b) derivation is shown on the following 2 pages.}
	\includepdf[pages=-]{lab1_1b.pdf}
	...
	\textcolor{b}{See \textbf{code/q1\_decode.py} for implementation and \textbf{result/decode\_output.txt} for results. Need to include "maximum objective value" here, and add comments in the q1\_decode.py file.}
\end{itemize}
...
\begin{itemize}
	...
	\textcolor{b}{See \textbf{code/q2\_gradient.py} for implementation and \textbf{result/gradient.txt} for results. Need to add comments to this file and, average log p(y|X) over training set value}
...
\textcolor{b}{See \textbf{code/q2\_gradient.py} for implementation and \textbf{result/solution.txt, result/prediction} for results. Need to add comments to this file and, include "optimal objective value found by solver".}
...
\textcolor{b}{The full folder for the SVM-HMM model and its executables is under \textbf{code/svm\_hmm} The script \textbf{code/q3\_convert\_to\_mc.py} was to convert the \_struct.txt files to \_mc.txt for the SVM-hmm model.}
...
\begin{itemize}
	...
	On the other hand, overly large \verb#-c# may lead to over-fitting, and make optimization challenging (taking a lot of time to converge).
	...
	\begin{minipage}{0.32\textwidth}
		\centering
		\includegraphics[width=\textwidth]{q3_crf_letter-wise.png}
	\end{minipage}
	\hfill
	\begin{minipage}{0.32\textwidth}
		\centering
		\includegraphics[width=\textwidth]{q3_svm_hmm_letter-wise.png}
	\end{minipage}
	\hfill
	\begin{minipage}{0.32\textwidth}
		\centering
		\includegraphics[width=\textwidth]{q3_svm_mc_letter-wise.png}
	\end{minipage}

	\textcolor{b}{\caption{Letter-wise accuracy comparison for CRF, SVM-HMM, and SVM-MC}}
	\end{figure}
	
	What observation can be made on the result?
	
	\textcolor{b}{The scripts used to train, predict, calulcate letter-wise accuracies and plot results for each C are located under \textbf{code/} as \textbf{q3\_crf.py, q3\_svm\_hmm.py, q3\_svm\_mc.py.}}
	
	\textcolor{b}{Looking at the letter-wise accuracy scores, all models have a similar curve and the improvements of accuracy drop off after c=1000. The gradual increase in accuracy is similar between all models. With CRF, the accuracy improvement is greater as it begins at \textasciitilde45\% and jumps as high as \textasciitilde85\% , while the SVM-HMM has an accuracy jump from \textasciitilde65\% to \textasciitilde85\%, and the SVC-MC from \textasciitilde48\% to \textasciitilde70\%. Ideal c value is 1000.} 
	...
	\begin{minipage}{0.32\textwidth}
		\centering
		\includegraphics[width=\textwidth]{q3_crf_word-wise.png}
	\end{minipage}
	\hfill
	\begin{minipage}{0.32\textwidth}
		\centering
		\includegraphics[width=\textwidth]{q3_svm_hmm_word-wise.png}
	\end{minipage}
	\hfill
	\begin{minipage}{0.32\textwidth}
		\centering
		\includegraphics[width=\textwidth]{q3_svm_mc_word-wise.png}
	\end{minipage}

	\textcolor{b}{\caption{Word-wise accuracy comparison for CRF, SVM-HMM, and SVM-MC}}
	\end{figure}

	\textcolor{b}{The scripts (same as 3a.) used to train, predict, calculate word-wise accuracies, and plot results for each C are located under (same files as 3a.) \textbf{code/} as \textbf{q3\_crf.py, q3\_svm\_hmm.py, q3\_svm\_mc.py.}}

	\textcolor{b}{Due to the SVM-MC treating every single letter as its own word and not accounting for transitions between letters, it makes sense to see the word accuracy is on average lower by about \textasciitilde30\% compared to CRF and SVM-HMM models. Similar to the curves for letter-wise accuracy across all models, there is a steady increase in accuracy as C increases here, with CRF jumping from \textasciitilde0\% to \textasciitilde50\%, SVM-HMM jumping from \textasciitilde15\% to \textasciitilde50\%, and SVM-MC jumping from \textasciitilde2\% to \textasciitilde17\%. Ideal c value remains 1000.}
...
\begin{itemize}
	...
	\textcolor{b}{The script \textbf{code/q4\_sdg.py} contains the code that runs SGD, momentum, and LBFGS optimizations and records the training objective value and word-wise error for the plots.}

	\begin{figure}[h]
    \centering
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{q4a_objective.png}
        \caption{\textcolor{b}{Training Objective score for SGD, Momentum, LBFGS over expected number of passes.}}
    \end{minipage}%
    \hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{q4a_word_error.png}
        \caption{\textcolor{b}{Word-wise Accuracy for SGD, Momentum, LBFGS over effective number of passes.}}
    \end{minipage}
	\end{figure}
	...
	\textcolor{b}{The work and explanation for 4bi is shown on the following page.}
	\includepdf[pages=-]{lab1_4b.pdf}
    ...
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{q4b_training-obj.png}
        \caption{\textcolor{b}{Training Objective score for SGD, Momentum, LBFGS over expected number of passes.}}
    \end{minipage}%
    \hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{q4b_word-error.png}
        \caption{\textcolor{b}{Word-wise Accuracy for SGD, Momentum, LBFGS over effective number of passes.}}
    \end{minipage}
	\end{figure}


	\textcolor{b}{In comparison to the convergence in 4a, applying MCMC sampling seems to limit the rate at which LBFGS converges. For both SGD and SGD Momentum, it looks like the MCMC sampling had little to no impact in improving the rate of convergence for both training objective and word-wise error.}
	...
	\textcolor{b}{The pseudocode is labeled as \textbf{Algorithm 1} on the next has been converted into latex readable pseudocode using the help of ChatGPT.}

	\begin{algorithm}
	\caption{Rao-Blackwellized Gibbs Sampling for CRF Marginals}

	\textbf{Initialization:}

	Initialize $y = (y_1, \dots, y_m)$ randomly\;
	Initialize node\_counts[$k$][$c$] $\gets 0$ for all $k,c$\;
	Initialize edge\_counts[$k$][$c,c'$] $\gets 0$ for all $k,c,c'$\;
	...
	\For{$iter = 1$ \KwTo $S$}{
		\For{$k = 1$ \KwTo $m$}{
			...
			Compute log conditional of $y_k$ given all other variables\;

			$log\_p[c] \gets \langle w_c, x_k \rangle$\;

			\If{$k > 1$}{
				$log\_p[c] \gets log\_p[c] + T[y_{k-1}, c]$\;
			}

			\If{$k < m$}{
				$log\_p[c] \gets log\_p[c] + T[c, y_{k+1}]$\;
			}

			Normalize to obtain conditional probability vector\;

			$p[c] \gets softmax(log\_p[c])$\;

			\BlankLine
			\textbf{Rao-Blackwell Update}

			\ForEach{$c$}{
				node\_counts[$k$][$c$] $\gets$ node\_counts[$k$][$c$] + $p[c]$\;
			}

			\If{$k > 1$}{
				edge\_counts[$k-1$][$y_{k-1},c$] $\gets$ edge\_counts[$k-1$][$y_{k-1},c$] + $p[c]$\;
			}

			\If{$k < m$}{
				edge\_counts[$k$][$c,y_{k+1}$] $\gets$ edge\_counts[$k$][$c,y_{k+1}$] + $p[c]$\;
			}

			Draw new sample: $y_k \sim \text{Categorical}(p)$\;
	...
	\For{$k = 1$ \KwTo $m$}{
		$\tilde{p}_k[c] = \dfrac{node\_counts[k][c]}{\sum_{c'} node\_counts[k][c']}$\;
	}

	\For{$k = 1$ \KwTo $m-1$}{
		$\tilde{p}_{k,k+1}[c,c'] =
		\dfrac{edge\_counts[k][c,c']}{\sum_{c,c'} edge\_counts[k][c,c']}$\;
	}
	...
	\textcolor{b}{The script to run MCMC with Rao-Blackwellization and without, and produce the corresponding KL divergence values across samples are in \textbf{code/q4c\_rao\_blackwell.py}.}
	
	\textcolor{b}{The KL divergence for the Node and Edge with RB has a faster convergence than the Node and Edge values without. Therefore, RB is effective at reducing variance. Another small piece to note is that Node marginals are easier to estimate than Edge marginals, but RB improves them both.}

	\begin{figure}[h]
	\centering
	\includegraphics[width=0.6\textwidth]{q4c_plot.png}
	\caption{\textcolor{b}{\textcolor{b}{Demonstrating the KL divergence across many samples}}}
	\end{figure}
...
\begin{itemize}
	...
	\textcolor{b}{The script \textbf{code/q5\_transform\_util.py} contains the utility functions used to apply transformations to the data. The script \textbf{code/q5\_driver.py} contains the loop that trains, predicts and records both letter-wise and word-wise accuracies for CRF and SVM-MC models across varying amounts of transformations.}
	...
	\begin{figure}[h]
	\centering
	\includegraphics[width=0.6\textwidth]{q5_letter-wise.png}
	\caption{\textcolor{b}{Letter-wise accuracy for CRF and SVM-MC across increasing transformations.}}
	\end{figure}

	What observation can be made on the result?
	
	\textcolor{b}{Both the CRF model and SVM-MC at c=1000 demonstrate a steady decrease in accuracy as there are more transformations applied to the data. Both also decrease in letter-wise accuracy by a net of around \textasciitilde10\%. There is a slight robustness to distortion, but the models are definitely affected.}
	...
	\begin{figure}[h]
	\centering
	\includegraphics[width=0.6\textwidth]{q5_word-wise.png}
	\caption{\textcolor{b}{\textcolor{b}{Word-wise accuracy for CRF and SVM-MC across increasing transformations.}}}
	\end{figure}

	What observation can be made on the result?

	\textcolor{b}{The SVM-MC underperforms compared to the CRF for word-wise scoring, which was explained earlier in question 3, however both models still do see a decrease in accuracies. The CRF drops by \textasciitilde20\%, and the SVM-MC drops by \textasciitilde10\%.}
